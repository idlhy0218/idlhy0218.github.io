...
In statistical analysis, "controlling" for a variable means accounting for its effect when examining the relationship between other variables. This helps us understand the true relationship between an independent variable (IV) and a dependent variable (DV) by removing the influence of potentially confounding factors.
...
::: {.callout-note}
### Key Points: Controlling Variables
...
$$\text{PriorKnowledge} \sim \text{Uniform}(1, 10)$$

$$\text{StudyHours} = 2 + 0.2 \times \text{PriorKnowledge} + \varepsilon_1, \quad \varepsilon_1 \sim N(0, 1)$$

$$\text{TestScore} = 30 + 3 \times \text{StudyHours} + 5 \times \text{PriorKnowledge} + \varepsilon_2, \quad \varepsilon_2 \sim N(0, 1)$$

Notice how prior knowledge influences both study hours and test scores, creating a confounding relationship.
...
### 3. Visualization: Understanding Control Through Stratification
...
4. **Parallel vs. Non-Parallel Lines**: If the colored lines were perfectly parallel, it would suggest prior knowledge only shifts scores up or down without changing how study hours affect scores. Non-parallel lines would suggest an interaction effect.

The mathematical reason for confounding bias can be illustrated using our model estimates:
...
This difference (`r round(coef_raw[2], 2)` - `r round(coef_controlled[2], 2)`) demonstrates how failing to control for confounding variables leads to overestimating effects. The bias in our uncontrolled estimate can be calculated as:
...
Another way to visualize controlling for a variable is through residualization:
...
::: {.callout-note}
### Residualization Process Summary
...
* Shows the X-Y relationship that cannot be explained by the control variable
:::
...
**Top Left Panel - Step 1**: Regress Study Hours on Prior Knowledge.
...
**Top Right Panel - Step 2**: Regress Test Score on Prior Knowledge.
...
- This relationship is mathematically equivalent to the coefficient of study hours in our multiple regression model
...
1. Isolates the unique effect of our independent variable on the dependent variable
2. Removes the influence of confounding factors that might bias our results
