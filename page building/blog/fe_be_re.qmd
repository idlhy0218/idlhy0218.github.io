...
Let's simulate some county health data to demonstrate these concepts. I made a simulated data with 100 counties over 10 years. I've picked two counties here. You can see there are both time varying and time invariant variables here. My research is to estimate the relationship between counties' income level (X) and mortality rate (Y):
...
1. **Top Left Panel**: Shows the raw data with county-specific trend lines fitted individually:
...
2. **Top Right Panel**: Shows the same data with fixed effects model fitted values:
...
3. **Bottom Panel**: Shows the data after the **"demeaning"** transformation:
...
set.seed(42)
selected_counties <- sample(1:n_counties, 9)
selected_data <- county_health_df %>%
  filter(county_id %in% selected_counties)
...
# Calculate predictions for each county
county_predictions <- selected_data %>%
  filter(county_id %in% selected_counties) %>%
  group_by(county_id) %>%
  mutate(
    # Fixed Effects Prediction (centered around county mean)
    fe_pred_full = mean(mortality_rate) + 
                   fe_coef * (median_income - mean(median_income)),
    
    # Between Effects Prediction
    be_pred_full = re_coef[1] + be_coef[2] * median_income,
    
    # Random Effects Prediction
    re_pred_full = re_coef[1] + re_coef[2] * median_income
  )

# Calculate county means
...
# Create a more informative plot with annotations
p6 <- ggplot() +
  ...
  # Fixed Effects lines with distinct styling
  geom_line(data = county_predictions, 
            aes(x = median_income, y = fe_pred_full, color = factor(county_id)), 
            linewidth = 1.2,
            linetype = "solid") +  # Fixed Effects
  
  # Between Effects lines with distinct styling
  geom_line(data = county_predictions, 
            aes(x = median_income, y = be_pred_full, color = factor(county_id)), 
            linewidth = 1.2, 
            linetype = "dashed") +  # Between Effects
  
  # Random Effects lines with distinct styling
  geom_line(data = county_predictions, 
            aes(x = median_income, y = re_pred_full, color = factor(county_id)), 
            linewidth = 1.5, 
            linetype = "dotted") +  # Random Effects
  
  # Mean points for each county with white border
...
# Create a legend box starting from x = 25
  annotate("rect", xmin = 25, xmax = 45, ymin = 910, ymax = 950,
           fill = "white", color = "gray", alpha = 0.7) +
  
  # Add annotations inside the box with proper spacing
  ...
  # Add diamond symbol explanation
  ...
  scale_color_brewer(palette = "Set1", name = "County ID") +
  scale_fill_brewer(palette = "Set1", name = "County ID") +
...
**County Mean Points**
- Diamond Shapes: Each county's average income and mortality; color-coded by county

**Model Lines**
- Solid Lines: Fixed effects with same slope but county-specific intercepts
- Dashed Lines: Between effects showing cross-sectional relationships
- Dotted Lines: Random effects demonstrating partial pooling; positioned between the other two

**Statistical Implications**
- Random effects "shrink" county estimates toward the global relationship when data is sparse or noisy
- Provides optimal balance between preserving local patterns and leveraging information from all counties
- The degree of shrinkage depends on within-county vs. between-county variance components
- Distance between fixed and random effects lines visually represents the amount of shrinkage applied
- Counties with more reliable data experience less shrinkage (random effects lines closer to fixed effects)
...
The ICC and the shrinkage parameter $\theta$ are intimately connected. Notice that the ICC formula closely resembles the $\theta$ formula:

$$ICC = \frac{\sigma^2_{\text{between}}}{\sigma^2_{\text{between}} + \sigma^2_{\text{within}}}$$

$$\theta = \frac{\sigma_u^2}{\sigma_u^2 + \sigma_\epsilon^2/T_i}$$

The key difference is that $\theta$ incorporates the number of observations per county ($T_i$), making it county-specific, while ICC is a global measure. When all counties have the same number of observations, the average $\theta$ approaches the ICC as $T_i$ increases.

This connection explains why:

1. With high ICC values, between-county differences dominate, so random effects estimates give more weight to the between estimator
2. With low ICC values, within-county variation dominates, so random effects estimates more closely resemble fixed effects estimates
3. The visualization shows this relationship - counties with more observations or less internal variance (higher $\theta$) have random effects lines closer to their fixed effects lines

Understanding this relationship helps us interpret our health data more effectively - we can see which counties' estimates are more heavily influenced by their own data versus the pooled data from all counties.
...
set.seed(42)
selected_counties <- sample(1:n_counties, 7)
selected_data <- county_health_df %>%
  filter(county_id %in% selected_counties)
...
p_icc <- ggplot() +
  ...
  scale_x_continuous(breaks = 1:7, labels = selected_counties) +
  scale_color_discrete(name = "County ID") 
