...
p1 <- ggplot(panel_df, aes(x = x, y = y, color = entity)) +
  ...
  labs(title = "Fixed Effects Model",
       subtitle = paste0("All entities share the same slope (β = ", round(fe_coef, 2), 
...
p2 <- ggplot(panel_df, aes(x = x_centered, y = y_centered, color = entity)) +
  ...
  labs(title = "Within-Entity Transformation",
       subtitle = "After subtracting entity means (demeaning), the common slope is revealed",
       x = "X - Entity Mean X", 
       y = "Y - Entity Mean Y") +
  theme_minimal() +
  theme(legend.position = "right",
...
p3 <- ggplot() +
  ...
  labs(title = "Between Effects Model",
       subtitle = "Focuses on differences between entity averages, ignoring within-entity variation",
       x = "X Variable", 
       y = "Y Variable") +
  theme_minimal() +
  theme(legend.position = "right",
