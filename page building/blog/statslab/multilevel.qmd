---
title: "Multilevel Models for Health Data Analysis"
author: "Heeyoung Lee"
...
## Understanding Multilevel Models in Health Research
...
In health economics and public health research, we frequently encounter multilevel structures such as:
...
Let's start by loading the necessary packages and creating a realistic multilevel health dataset:
...
Our multilevel dataset contains `r nrow(county_data)` counties nested within `r n_states` states, with realistic distributions based on actual U.S. health data patterns. The data structure includes:
...
Before diving into multilevel models, it's crucial to understand why standard regression fails with nested data. The **intracluster correlation coefficient (ICC)** quantifies how similar counties are within states compared to counties in different states.

```{r icc-analysis}
# Calculate descriptive statistics by state
state_summary <- county_data %>%
...
state_summary %>%
  mutate(
    medicaid_status = ifelse(medicaid_expansion == 1, "Expanded", "Not Expanded")
  ) %>%
  dplyr::select(state_name, n_counties, mean_mortality, sd_mortality, 
         mean_income, medicaid_status) %>%
  kable(digits = 1, caption = "Mortality Patterns by State") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  column_spec(1, bold = TRUE) %>%
  row_spec(which(state_summary$medicaid_expansion == 1), background = "#E8F5E8")
```

```{r icc-visualization, fig.width=12, fig.height=8}
# Calculate overall ICC using ANOVA approach
mortality_anova <- aov(mortality_rate ~ factor(state_id), data = county_data)
icc_anova <- summary(mortality_anova)

# Extract variance components
ms_between <- icc_anova[[1]]$`Mean Sq`[1]  # Between-state variance
ms_within <- icc_anova[[1]]$`Mean Sq`[2]   # Within-state variance
n_bar <- county_data %>% count(state_id) %>% pull(n) %>% mean() # Average counties per state

# Calculate ICC
sigma2_between <- (ms_between - ms_within) / n_bar
sigma2_within <- ms_within
icc <- sigma2_between / (sigma2_between + sigma2_within)

# Create visualization showing state clustering
p1 <- ggplot(county_data, aes(x = reorder(state_name, mortality_rate), 
                              y = mortality_rate, color = factor(state_id))) +
  geom_jitter(width = 0.2, alpha = 0.6, size = 2) +
  stat_summary(fun = mean, geom = "point", size = 4, 
               shape = 23, fill = "red", color = "black") +
  stat_summary(fun = mean, geom = "line", aes(group = 1), 
               color = "red", linewidth = 1.2) +
  labs(title = "County Mortality Rates Clustered by State",
       subtitle = paste0("ICC = ", round(icc, 3), 
                        " (", round(100*icc, 1), "% of variance is between states)"),
       x = "State (ordered by mean mortality)", 
       y = "County Mortality Rate (per 100,000)",
       caption = "Red diamonds show state means; line connects state means") +
  theme_minimal() +
...
p2 <- ggplot(county_data, aes(x = reorder(state_name, mortality_rate), 
                              y = mortality_rate, fill = factor(medicaid_expansion))) +
  ...
  labs(title = "State-Level Policy Effects on County Mortality",
       subtitle = "Counties within states share policy environments",
       x = "State (ordered by mean mortality)", 
       y = "County Mortality Rate (per 100,000)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
**Figure Interpretation:** The visualizations reveal critical patterns in our health data:

The **top panel** shows counties clearly cluster by state, with red diamonds indicating state means that vary considerably. The ICC of `r round(icc, 3)` indicates that `r round(100*icc, 1)`% of the total variance in mortality rates occurs between states rather than within states. This substantial clustering means that ignoring the multilevel structure would lead to incorrect standard errors and potentially misleading conclusions.

The **bottom panel** demonstrates how state-level policies create systematic differences. States with Medicaid expansion (blue boxes) tend to have lower mortality rates than non-expansion states (red boxes), though there's still considerable within-state variation. This pattern suggests that both state-level policies and county-level factors contribute to health outcomes, necessitating a multilevel modeling approach.

### 3. The Multilevel Model Framework

#### 3.1 Basic Two-Level Model Structure
...
Got it — you’d like me to add more **explanation of what each centering approach means** in practice, so your students/readers can see why the choice matters before diving into contextual effects. Here’s how you can expand your text around **6.1 The Centering Problem** without changing your code chunks:
...
#### When to Use Multilevel Models in Health Research
