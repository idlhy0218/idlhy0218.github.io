...
## Why is TWFE Biased?
**Reference**: [Goodman-Bacon (2021)](https://doi.org/10.1016/j.jeconom.2021.03.014)
...
- [did package vignette](https://cran.r-project.org/web/packages/did/vignettes/did-basics.html)
...
- [fixest](https://lrberge.github.io/fixest/reference/sunab.html)
...
# 3-3. Method 3: Wooldridge ETWFE (2023)

**Reference**: 

- [etwfe package vignette](https://cran.r-project.org/web/packages/etwfe/vignettes/etwfe.html)
...
Alternative specification using never-treated units as controls. This returns pre-treatment effects.
...
- [did2s GitHub](https://github.com/kylebutts/did2s)
- [Gardner (2022)](https://doi.org/10.48550/arXiv.2207.05943)
...
### Custom Event Study Plot (ggplot)
...
ggplot(results, aes(x = Method, y = ATT)) +
  ...
  labs(title = "Method Comparison", 
       subtitle = "Red line = true effect (5.36)",
       x = "", y = "ATT Estimate") +
  theme_minimal()
```
...
```{r event-study-comparison}
## Combined Event Study Plot
# --- TRUE EFFECT LINE ---
true_es <- data %>%
  filter(treated == 1) %>%
  mutate(rel_time = time - treat_time) %>%
  group_by(rel_time) %>%
  summarise(true_effect = mean(true_te), .groups = "drop")

# --- Extract event study coefficients (same as before) ---
...
# --- Plot comparison with TRUE line ---
ggplot(all_es, aes(x = rel_time, y = estimate, color = method, shape = method)) +
  ...
  geom_line(data = true_es, aes(x = rel_time, y = true_effect),
            inherit.aes = FALSE, color = "black", linewidth = 1, linetype = "dotted") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", alpha = 0.5) +
  geom_vline(xintercept = -0.5, linetype = "dotted", alpha = 0.3) +
  scale_color_manual(values = c(
    "TWFE (biased)" = "#C44536",
    "Callaway & Sant'Anna" = "#4A6FA5",
    "Sun & Abraham" = "#588B8B",
    "Wooldridge ETWFE" = "#8E7C93",
    "Gardner Two-Stage" = "#D4A574"
  )) +
    scale_shape_manual(values = c(
    "TWFE (biased)" = 16,
    "Callaway & Sant'Anna" = 17,
    "Sun & Abraham" = 15,
    "Wooldridge ETWFE" = 18,
    "Gardner Two-Stage" = 19
  )) +
  labs(
    title = "Event Study Comparison: All Methods vs. True Effect",
    subtitle = "Black dotted line indicates true effects",
